...
export default class 组织部门 extends XBasePage {
    ...
    showSaveModal组织部门(data, parent) {
        ...
        const Ele = (
            <XForm infoUrl={"zzbm/queryinfo"} useServerInfo={true} infoData={data} inited={(e) => this.form = e}>
                <XGrid columnGap={"10px"} rowGap={"10px"} rowsTemplate={["auto"]}>
                    ...
                    <XInput ref={e => this.input详细地址 = e} labelWidth={labelWidth} field={"详细地址"} visible={false}
                            parent={() => this.form}/>
                    <XInput ref={e => this.input法定代表人 = e} labelWidth={labelWidth} field={"法定代表人"}
                            visible={false}
                            parent={() => this.form}/>
                    <XInput ref={e => this.input开户行 = e} labelWidth={labelWidth} field={"开户行"}
                            parent={() => this.form}
                            visible={false}/>
                    <XInput ref={e => this.input银行账号 = e} labelWidth={labelWidth} field={"银行账号"}
                            parent={() => this.form}
                            visible={false}/>
                    <XInput ref={e => this.input公司税号 = e} labelWidth={labelWidth} field={"公司税号"}
                            parent={() => this.form}
                            visible={false}/>
                </XGrid>
            </XForm>);
    ...
    render() {
        return <XCard paddingTRBL={"10px"}>
            <XTableGrid inited={(e) => this.table组织部门 = e} dataSourceUrl="zzbm/querylist" showSearch={false}
                        isTree={true} onSelectChange={(row) => this.select组织部门 = row} draggable={true}
